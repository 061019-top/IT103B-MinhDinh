...
const sumGoals = (minGoals, list) => {
    let filterList = list.filter((item) => {
        return item.goals >= minGoals;
    });

    let total = filterList.reduce((acc, cur) => {
...
let minGoal = +prompt("Nhập bàn thắng tối thiểu");
console.log(`Tổng các bàn thắng của các cầu thủ đạt đủ chỉ tiêu là : ${sumGoals(minGoal, players)}`);